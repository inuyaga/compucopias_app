...
import 'package:compucopias_app/widgets/aboutDialog.dart';
import 'package:http/http.dart' as http;
import 'package:compucopias_app/conexionesString.dart';
import 'package:compucopias_app/objetos.dart';
      ...
      child: ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            return Card(
              elevation: 5,
              child: ListTile(
                title: Text(compras[index].cwStatus,
                ...
                leading: SizedBox(
                    ...
                    child: Center(
                      child: Text(
                        "N° ${compras[index].cwId}",
                        style:
                            TextStyle(color: ColoresApp.primario, fontSize: 10),
                      ),
                    )),
                subtitle: Text(
                  "${compras[index].cwFecha}",
                  style: TextStyle(fontFamily: 'KG'),
                ),
                trailing: Text(
                  "${oCcy.format(compras[index].total + 160.0)}",
                  style: TextStyle(color: ColoresApp.defecto, fontFamily: 'KG'),
                ),
                onTap: () {
                  Navigator.of(context).push(MaterialPageRoute(
                      builder: (context) => DetalleCompraWebView(
