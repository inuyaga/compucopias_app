...
import 'package:compucopias_app/paginas/cambiarDomicilio.dart';
import 'package:compucopias_app/paginas/finalizarCompra.dart';
...
class _ShopingCarViewState extends State<ShopingCarView> {
  ...
  Future<Widget> getTextCp() async {
    ...
    String token = preferencias.getString("token") ?? "";
    final response =
        await http.get(InternetString.domiciliosUserActivo, headers: {
      HttpHeaders.authorizationHeader: 'Token $token',
    });
    ...
    return Text(
      dom == null
          ? "Seleccione lugar de envio"
          : "Enviar a C.P: ${dom.domcodigop}",
